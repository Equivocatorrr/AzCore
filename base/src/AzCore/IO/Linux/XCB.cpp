...
void SetCursorXCB(Window *window) {
	if (window->cursorHidden) {
		u32 value = window->data->x11.cursorHidden;
		xcb_change_window_attributes(window->data->x11.connection, window->data->x11.window, XCB_CW_CURSOR, &value);
	} else {
...
bool xkbUpdateKeymapXCB(xkb_keyboard *xkb) {
	// Cleanup first
	if (xkb->keymap) {
		xkb_keymap_unref(xkb->keymap);
	}
...
bool xkbSetupKeyboardXCB(xkb_keyboard *xkb, xcb_connection_t *connection) {
...
bool windowOpenX11(Window *window) {
	...
	if (!xkbSetupKeyboardXCB(&data->xkb, data->x11.connection)) {
