...
struct BucketArray {
    ...
    BucketArray() : size(0) {}
    BucketArray(const i32 newSize) : size(newSize) {}
    BucketArray(const i32 newSize, const T& value) : size(newSize) {
        for (i32 i = 0; i < size; i++) {
            data[i] = value;
        }
    }
    BucketArray(const u32 newSize) : BucketArray((i32)newSize) {}
    BucketArray(const u32 newSize, const T& value) : BucketArray((i32)newSize, value) {}
    ...
    inline T& operator[](const i32 index) {
#ifndef MEMORY_NO_BOUNDS_CHECKS
        if (index > size) {
            throw std::out_of_range("BucketArray index is out of bounds");
        }
    ...
    inline void Resize(const i32 newSize, const T& value) {
        for (i32 i = size; i < newSize; i++) {
            data[i] = value;
        }
        size = newSize;
    ...
    inline void Resize(const i32 newSize) {
        size = newSize;
    }
