...
void main() {
	vec2 pos = pc.transform * (inPosition-pc.origin) + pc.position.xy;
	float z = pc.z + pc.zShear * pos.y;
	gl_Position = vec4(pos, 0.0, 1.0);
	outTexCoord = inTexCoord;
	outScreenPos = (vec3(pos, z-1.0) + 1.0) * 0.5 * vec3(ub.screenSize, ub.screenSize.y);
