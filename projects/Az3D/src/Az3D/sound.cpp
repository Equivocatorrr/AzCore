...
bool Manager::Initialize() {
	AZCORE_PROFILING_SCOPED_TIMER(Az3D::Sound::Manager::Initialize)
...
bool Manager::Deinitialize() {
	AZCORE_PROFILING_SCOPED_TIMER(Az3D::Sound::Manager::Deinitialize)
...
Array<PriorityIndex> Manager::GetPriorities() {
	AZCORE_PROFILING_SCOPED_TIMER(Az3D::Sound::Manager::GetPriorities)
...
bool Manager::UpdateActiveSound(SourceBase *sound, f32 timestep) {
	AZCORE_PROFILING_SCOPED_TIMER(Az3D::Sound::Manager::UpdateActiveSound)
	...
	alSourcef(sound->source, AL_PITCH, sound->pitch * (sound->simulationPitch ? sys->simulationRate : 1.0f));
	if (!ErrorCheck("alSourcef(AL_PITCH)")) return false;
	f32 gain = sound->gain * (f32)Settings::ReadReal(Settings::sVolumeMain);
	switch(sound->channel) {
		case MUSIC:
			gain *= (f32)Settings::ReadReal(Settings::sVolumeMusic);
			break;
		case FX:
			gain *= (f32)Settings::ReadReal(Settings::sVolumeEffects);
			break;
	}
	alSourcef(sound->source, AL_GAIN, gain * sound->fadeoutFactor);
	...
	if (state == AL_PLAYING) {
		...
		if (sound->stop) {
			if (sound->fadeoutTime > 0.0f && sound->fadeoutFactor > 0.0f) {
				sound->fadeoutFactor = max(0.0f, sound->fadeoutFactor - timestep/sound->fadeoutTime);
			} else {
...
bool Manager::Update(f32 timestep) {
	AZCORE_PROFILING_SCOPED_TIMER(Az3D::Sound::Manager::Update)
	if (procFailure) {
		return false;
	}
	...
	for (i32 i = 0; i < min(maxSources, priorities.size); i++) {
		...
		if (sound->active) {
			if (!UpdateActiveSound(sound, timestep)) goto failure;
		}
	}
	soundMutex.Unlock();
	return true;
...
void Manager::FadeoutAll(f32 fadeoutDuration) {
	soundMutex.Lock();
	for (i32 i = 0; i < sounds.size; i++) {
		SourceBase *sound = sounds[i];
		if (sound->active && sound->playing) {
...
void MultiSource::Play(f32 gain, f32 pitch) {
	Stop();
	current = shuffle(shuffleId, sources.size);
...
void MultiSource::Stop(f32 fadeoutDuration) {
	if (current >= 0) {
		sources[current]->Stop(fadeoutDuration);
	}
}
...
bool Stream::SetLoopRange(i32 begin, i32 end) {
	...
	if (end == -1) end = file->data.totalSamples-1;
