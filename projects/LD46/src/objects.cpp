...
void Manager::Sync() {
    buffer = !buffer;
    if (!paused) {
        globals->objects.simulationRate = min(1.0f, globals->objects.simulationRate + globals->objects.timestep * 2.0f);
    } else {
        globals->objects.simulationRate = max(0.0f, globals->objects.simulationRate - globals->objects.timestep * 2.0f);
    }
